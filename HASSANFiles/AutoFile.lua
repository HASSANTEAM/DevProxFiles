...
DevAbs:del(HASSAN.."Abs:Lock:AutoFile")
end
...
DevAbs:set(HASSAN.."Abs:Lock:AutoFile",true) 
end 
end

if (text and not DevAbs:get(HASSAN.."Abs:Lock:AutoFile")) then
Time = DevAbs:get(HASSAN.."Abs:AutoFile:Time")
if Time then 
if Time ~= os.date("%x") then 
local list = DevAbs:smembers(HASSAN..'Abs:Groups') 
local BotName = (DevAbs:get(HASSAN.."Abs:NameBot") or 'بروكس')
...
BasicConstructors = DevAbs:smembers(HASSAN..'Abs:BasicConstructor:'..v)
Constructors = DevAbs:smembers(HASSAN..'Abs:Constructor:'..v)
...
local curl = 'curl "' .. abbas .. '" -F "chat_id='..DevId..'" -F "document=@'..HASSAN..'.json' .. '" -F "caption=⌁︙نسخه تلقائيه تحتوي على ↫ '..#list..' مجموعه"'
io.popen(curl)
io.popen('fm -fr '..HASSAN..'.json')
DevAbs:set(HASSAN.."Abs:AutoFile:Time",os.date("%x"))
end
else 
DevAbs:set(HASSAN.."Abs:AutoFile:Time",os.date("%x"))
end
